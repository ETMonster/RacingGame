...
trial_npc=Opponent(
    image=pygame.image.load('red_car.png'),  # Car image path
    width=pygame.image.load('red_car.png').get_width() * PIXEL_TO_SCREEN_FACTOR,
    # Car width so it retains its aspect with rescale
    height=pygame.image.load('red_car.png').get_height() * PIXEL_TO_SCREEN_FACTOR,
    # Car height so it retains its aspect with rescale
    position=Vector(x=WINDOW_WIDTH // 2, y=WINDOW_HEIGHT // 2),  # Position on screen
    velocity=Vector(x=0, y=0),
    # Forward velocity (positive if moving forward from the car's point of view, negative if moving backwards from car's point of view)
    direction=Vector(x=0, y=0),
    # The direction that the car is currently moving in (-1 to 1) and multiplies by velocity to get new position
    rotation=Rotation(radians=0, angle=0),  # The current rotation of the car in radians and angles
